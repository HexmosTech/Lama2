...
const Explorer: FC<any> = () => {
  ...
  const directoryToWatch = '/home/lince/hexmos/apihub/apihub';
  const fileExtensionToWatch = '.l2';

  const folderPattern = path.join(directoryToWatch, '*');
  try {
    watch(folderPattern, { depth: 0 }) // Set depth to 0 to only watch the immediate directory
      .on('addDir', (folderPath) => {
        ...
        // Filter and log only files with the specified extension
        filesInFolder
          .filter((fileName) => fileName.endsWith(fileExtensionToWatch))
          .forEach((filteredFileName) => {
            console.log(`File name inside ${folderName}: ${filteredFileName}`);
          });
      })
      .on('error', (error) => {
  ...
  // useEffect(() => {
  //   // Set the directory or file to watch
