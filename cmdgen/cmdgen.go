...
func ConstructWasmCommand(parsedInput *gabs.Container) ([]string, string) {
	log.Info().Str("ParsedInput", parsedInput.String()).Msg("")
	httpv := parsedInput.S("verb", "value")
	url := parsedInput.S("url", "value")
	jsonObj := parsedInput.S("details", "ip_data")
	...
	res, stdinBody := assembleWasmCmdString(httpv.Data().(string), url.Data().(string), jsonObj, headers, multipartBool)
	log.Info().Interface("res", res).Msg("Response")
	return res, stdinBody
}

func assembleWasmCmdString(httpv string, url string, jsonObj *gabs.Container, headers *gabs.Container, multipart bool) ([]string, string) {
	command := make([]string, 0)
	log.Info().
		Str("Type", "Construct Command").
		Str("httpv", httpv).
		Str("url", url).
		Bool("multipart", multipart).
		Msg(fmt.Sprint("Construct parameters 123"))
	...
	log.Info().Interface("commands", cleanCommand).Msg("Logging commands")
	return cleanCommand, jsonStr
}
