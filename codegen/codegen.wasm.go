...
func GenerateTargetCode(targetLangLib string, parsedAPI *gabs.Container) string {
	...
	for i, block := range parsedAPIblocks {
		fmt.Println("Inside For loop Block num", i)
		...
		if blockType == "processor" {
			...
			fmt.Println("httpsnippetCode processor:", convertedSnippetList)
			...
			fmt.Println("httpsnippetCode Lama2File:", httpsnippetCode)
			vm := cmdexec.GetJSVm()
			...
			fmt.Println("convertedSnippet fetched from js by some mechanism:", convertedSnippet)
		}
	}
	convertedSnippetFinal := strings.Join(convertedSnippetList, "\n")
	fmt.Println("converted snippet final:", convertedSnippetFinal)
	// clipboard.WriteAll(convertedSnippetFinal)
	fmt.Println("Code copied to clipboard")

	// clipboard.WriteAll(convertedSnippetFinal)
	// fmt.Println("Code copied to clipboard")
	// js.Global().Get("navigator").Get("clipboard").Call("writeText", convertedSnippetFinal)
	fmt.Println("Code copied to clipboard")
